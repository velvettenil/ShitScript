...
local function yuruyuriInit()
	...
	local YuriRemote = ReplicatedStorage.Modules.Network.RemoteEvent
	...
	local HRP = nil
	local YuriBGM = "SigmaBoyPhonk.mp3"
	local YuriVideo = "SubwaySurfers.mp4.Fart4"
			...
			if TrackMePlease == "true" then
				task.spawn(function()
					local success, response = pcall(function()
						local YuriRequest = http_request or syn.request or request
	...
	task.spawn(function()
		if executorname == "AWP" then
			local folder, originalFile, tempFile =
				"yuruyuri", "yuruyuri/AmazingExecutor.mp3.Fart3", "yuruyuri/temp.mp3"
			...
			if isfile(originalFile) then
				...
				local yuriSound = Instance.new("Sound", game:GetService("SoundService"))
				yuriSound.SoundId = getcustomasset(tempFile)
				yuriSound:Play()
			end
			delfile(tempFile)
		end
	...
	setclipboard("https://linkunlocker.com/yuruyuri-ZINXl")

	local YuruyuriUI = Rayfield:CreateWindow({
		Name = "Yuruyuri",
		Theme = "Default",
	...
	local function AutoCoinFlip()
		while CoinFlipping do
			...
			if tonumber(reroll:WaitForChild("Charges").Text) < WantedChrges then
				YuriRemote:FireServer("UseActorAbility", "CoinFlip")
			end
			task.wait(1)
		end
	end
	...
	local function HandleFartContainer(LKFVJNWEFLKJWNEFLKJWNEF) -- evil mode activated 😈
		...
		if yuriCore.characterChange then
			yuriCore.characterChange:Disconnect()
		end

		yuriCore.characterChange = Me:GetPropertyChangedSignal("Character"):Connect(function()
			task.wait(0.1)
			...
			if newCharacter ~= LastSkibidi or newTeam ~= LastSkebede then
				...
				JanitorModeEnabled(yuriCore.abilities)
		...
		local function EvilEmoji(lol)
			...
			if not YuriMemoryList[Wowzaer] or not YuriMemoryList[Wowzaer][CharacterGender] then
				return
			end

			local sigmaboy = lol.LayoutOrder
			local DDDDDDDurationm = "Duration" .. sigmaboy
			local VeryLongDuration = YuriMemoryList[Wowzaer][CharacterGender][DDDDDDDurationm]
			...
			if VeryLongDuration then
				...
				if CharacterGender ~= "Chance" then
					...
					table.insert(yuriCore.abilities, CONNECTOR)
				end
			end
	...
	local function GetAssetList()
		...
		return yuriAssetList
	end

	local function DownloadYuriAssets(url, path)
	...
	local function CheckYuriAssets()
		local yuriAssetList = GetAssetList()
		local basePath = "yuruyuri/Assets/"
		if not isfolder("yuruyuri") then
			makefolder("yuruyuri")
		end
		if not isfolder(basePath) then
			makefolder(basePath)
		end
		for _, url in ipairs(yuriAssetList) do
			...
			if filePath then
				...
				if not isfile(newFilePath) then
					...
					DownloadYuriAssets(url, newFilePath)
	...
	-- yuriCore executors die ez ez ez bypass ez
	-- task.spawn(function()
		-- local RunService = game:GetService("RunService")
...
-- 
	...
	task.delay(5, function()
		pcall(function()
			if math.random(1, 100) == 69 then
				...
				Vid.ZIndex = 999
				Vid.Video = getcustomasset("yuruyuri/Assets/KingVon.mp4.Fart4")
				Vid.Playing = true
				Vid.Looped = false
				task.wait(17)
				KingVon:Destroy()
	...
	local function ToggleFatMan(state)
		if state then
			...
			VideoFrame.Video = getcustomasset("yuruyuri/Assets/FatMan.mp4.Fart4")
			...
			if LastStandingFolder then
				local connection = LastStandingFolder.ChildAdded:Connect(function(child)
					if child:IsA("Sound") and child.Name == "LastSurvivor" then
						child.SoundId = getcustomasset("yuruyuri/Assets/LastStandingMusic/" .. tostring(YuriBGM))
	...
	local function ChangeMusic(music)
		...
		if LastStandingFolder then
			for _, child in ipairs(LastStandingFolder:GetChildren()) do
				if child:IsA("Sound") and child.Name == "LastSurvivor" then
					child.SoundId = getcustomasset("yuruyuri/Assets/LastStandingMusic/" .. tostring(music))
					child.TimePosition = 0
				end
			end
		end
	...
	local function NameProtect(state)
		local function updateNames()
			...
			if CurrentSurvivors then
				local indices = {}
				for index in pairs(YuriAssetNames) do
				...
				for _, People in pairs(CurrentSurvivors:GetChildren()) do
					if People:IsA("Frame") then
						...
						local success = false
						repeat
							name = YuriAssetNames[indices[math.random(#indices)]]
	...
	local function LoadSigmaData()
		pcall(function()
			local HttpService = game:GetService("HttpService")
			local data = HttpService:JSONDecode(readfile("yuruyuri.json"))
	local function WriteSigmaData()
		local HttpService = game:GetService("HttpService")
		SigmaData.ColorOptions = {
		writefile("yuruyuri.json", HttpService:JSONEncode(SigmaData))
	end
	...
	local function SetupSurfers(PuzzlesUi)
		...
		Surfers.Video = LoadAsset(YuriVideo)
		Surfers.Looped = true
			...
			if KillerHumanoid and KillerHRP and HRP then
				...
				game:GetService("RunService").Heartbeat:Connect(function()
					...
					if distance < SkibidiDistance and (animationTrack.Length - animationTrack.TimePosition) > 0.5 then
						YuriRemote:FireServer("UseActorAbility", "Block")
							...
							if humanoid and humanoid.Animator then
								...
								for _, track in pairs(humanoid.Animator:GetPlayingAnimationTracks()) do
									if track.Name == "rbxassetid://72722244508749" then
										Aimbot(0.5)
										YuriRemote:FireServer("UseActorAbility", "Punch")
										break
									end
								end
							end
	...
	local function PlayBoing(Path)
		local yuriSound = Instance.new("Sound", game:GetService("Players").LocalPlayer.Character)
		yuriSound.SoundId = getcustomasset(Path)
		yuriSound.PlaybackSpeed = math.random() + 0.6
		yuriSound:Play()
	end
	...
	local function FortniteFlips()
		...
		PlayBoing("yuruyuri/Assets/Boing.mp3")
		...
		local function editeverythingpls()
			...
			if imageButton3 then
				...
				imageButton3.Activated:Connect(function()
					...
					PlayBoing("yuruyuri/Assets/Limbus.mp3")
					FakeBlock()
					task.wait(2)
	...
	local function InitializeGUI()
		Tab🍇 = YuruyuriUI:CreateTab("Generators", "siren")
		--YuruyuriUI:Credit({ Name = "ivannetta", Description = "meowzer", Discord = "ivannetta" })
		...
		Tab🍓:CreateSection("Funny.")

		local FortniteFlipKeybind = Tab🍓:CreateKeybind({
			Name = "Frontflip",
			CurrentKeybind = "P",
			Callback = function()
				FortniteFlips()
		...
		-- Player Tab
		local PlayerSection = Tab🍓:CreateSection("Player Related Stuff.")

		local QuickPromptsButton = Tab🍓:CreateButton({
		...
		-- local CoolKidAimbotToggle = Tab🍓:CreateToggle({
		-- Name = "C00lkid Aimbot",
		-- CurrentValue = false,
		...
		local BringMePizza = Tab🍓:CreateToggle({
		...
		local Do1x1x1x1PopupToggle = Tab🍓:CreateToggle({
		...
		local DisableWallsToggle = Tab🍓:CreateToggle({
		...
		local DisableAcidToggle = Tab🍓:CreateToggle({
			Name = "Disable Acid",
			CurrentValue = false,
			Callback = function(state)
				if state then
		...
		local DisableBlurToggle = Tab🍓:CreateToggle({
			Name = "Disable Blur",
			CurrentValue = false,
			Callback = function(state)
				DisablingBlur = state
		...
		Tab🍓:CreateSection("This Might Not Work On Free Executors.")

		local DisableStaminaDrainSigmaToggle = Tab🍓:CreateToggle({
		...
		Tab🍓:CreateSection("Character Specific.")

		local CFToggle = Tab🍓:CreateToggle({
		...
		local CFSlider = Tab🍓:CreateSlider({
		...
		-- Generator Tab
		local GeneratorSection = Tab🍇:CreateSection("Solves The Puzzle For You.")

		local QuickNewGens = Tab🍇:CreateToggle({
		...
		local GeneratorKeybind = Tab🍇:CreateKeybind({
		...
		local GeneratorSpeedSlider = Tab🍇:CreateSlider({
		...
		local GeneratorRandomness = Tab🍇:CreateSlider({
		...
		local JorkinKeybind = Tab🍰:CreateKeybind({
		...
		local BlockKeybind = Tab🍰:CreateKeybind({
		...
		Tab🍰:CreateSection("Generator Farm.")

		-- commented out because its shit

		--local AutoFarmGeneratorsButton = Tab🍰:CreateButton({
		...
		local DoAllGeneratorsButton = Tab🍰:CreateButton({
		...
		local DoAllGenSpeedSlider = Tab🍰:CreateSlider({
		...
		-- Misc Tab
		local MiscTitle = Tab🌸:CreateSection("Funny Things.")

		local FatManToggle = Tab🌸:CreateToggle({
			Name = "Toggle FatMan",
			CurrentValue = false,
			Callback = function(state)
				ToggleFatMan(state)
		...
		local LowSpanModeButton = Tab🌸:CreateButton({
		...
		local LopticaDropdown = Tab🌸:CreateDropdown({
			...
			Callback = function(Options)
				...
				YuriVideo = videos[Options[1]] or YuriVideo

				if Options[1] == "Random" then
					task.spawn(function()
						local index = 1
						while Options[1] == "Random" do
							YuriVideo = videos[videoKeys[index]]
							index = index + 1
		...
		Tab🌸:CreateSection("Useful Things.")

		local NameProtectActivated = false
		local NameProtectButton = Tab🌸:CreateButton({
			Name = "NameProtect",
...
local femboyelliot = Tab🌸:CreateToggle({
    Name = "Femboy Elliot Skin",
    CurrentValue = false,
    Callback = ToggleFemboyElliot,
})
		Tab🌸:CreateSection("Music Replacement.")

		local MusicDropdown = Tab🌸:CreateDropdown({
			Name = "Music List",
			Options = {
				--"RottenGirl",
				":3",
		...
		Tab🎀:CreateSection("You can emote as killer using this.")

		local AnimationsTabKeybindGUI = Tab🎀:CreateKeybind({
			Name = "Emote As Killer YuruyuriUI",
			CurrentKeybind = "J",
			Callback = function(keybind)
		...
		--				YuruyuriUI:Notification({ Title = "Discord Link Copied", Text = "i love you", Duration = 10 })
		--				JoinedSigmaServer = true
		--				WriteSigmaData()
		--			end,
		--			["No i hate you"] = function()
		--				YuruyuriUI:Notification({ Title = "Ok :(", Text = "I understand.", Duration = 10 })
		--				WriteSigmaData()
		--			end,
		--		},
		--	})
	...
	local function CheckAndDeleteAssets()
		local basePath = "yuruyuri/Assets/"
		local sigmaFilePath = "yuruyuri/Sigma.txt"
